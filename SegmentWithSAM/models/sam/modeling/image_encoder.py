...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        if self.args.if_split_encoder_gpus:
            self.patch_embed = self.patch_embed.to(self.dev)

        self.pos_embed: Optional[nn.Parameter] = None
        if use_abs_pos:
            # Initialize absolute positional embedding with pretrain image size.
            self.pos_embed = nn.Parameter(
                torch.zeros(1, img_size // patch_size, img_size // patch_size, embed_dim,dtype=torch.float,device=self.dev))
        ...
        if self.args.if_split_encoder_gpus:
            self.neck = self.neck.to(self.dev)
